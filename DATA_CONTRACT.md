...
FILLED recovery requires matching terminal Binance order evidence and canonical
physical `myTrades`; response quantities or prices alone are insufficient.
Canonical order/client/trade identities are retained, and retries reuse the
existing transaction and durable identities. Unavailable or contradictory
evidence remains unresolved and never invents inventory or FIFO mutations.
Terminal non-FILLED orders are not resolved from status or order summaries;
...
Within `quantity_path`, `intended_quantity` and
`quantity_before_normalization` represent the pre-normalization SELL intent;
`origQty` is not used for that meaning because Binance normally reports it as
the submitted normalized order quantity. `normalized_submitted_quantity` and
`executed_quantity` retain their distinct submitted and executed semantics.
...
Semantics for `RECOVER_BOT_FILLED_SELL`:

- Used only for a bot-owned Binance SELL already confirmed `FILLED`.
- The request payload must contain the canonical order (`orderId`,
  `clientOrderId`, `status`) and canonical `myTrades` rows with non-null
  `tradeId`; missing identities are rejected.
- The executed base and quote quantities are derived from canonical fills.
- Physical Binance fills, commission evidence, one linked FILLED SELL
  operation, and FIFO lot closures are persisted by the bot service.
- Only the executed quantity is closed; residual open-lot quantity remains.
- Existing operation or fill identity for the Binance order rejects the apply.
- This is accounting recovery only: no Binance order is submitted.
