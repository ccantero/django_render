...
- Telegram `/health` includes Disk Usage from latest
  `bot.bot_healthcheck.details.disk_usage`, labels the source, and degrades to
  `Unavailable` without hiding the rest of the health message when the bot
...
- Disk Usage card on the authenticated dashboard and Telegram `/health`, sourced
  from latest `bot.bot_healthcheck.details.disk_usage`. Missing or malformed
  bot-persisted disk data renders `Unavailable`; Django does not measure VPS
  disk directly and does not show Django/Render host disk usage as VPS health.
...
- `/health/` only confirms that the Django web process is reachable; it is not a bot/database health check. Dashboard and Telegram Disk Usage are separate operator diagnostics sourced from latest bot healthcheck details and do not persist data in Django.
