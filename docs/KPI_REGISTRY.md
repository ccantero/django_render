...
| `portfolio_status_change_24h_7d_30d` | Diagnostic | Historical equity change for compact Telegram review. | Latest reliable canonical snapshot equity minus the closest reliable historical canonical snapshot inside the horizon tolerance window, with percentage over historical equity. Tolerances are 18-30h for 24h, 6-8d for 7d, and 28-32d for 30d. | `bot.portfolio_snapshots.notes.portfolio_equity_usdt` only | Django `/portfolio_status` | Telegram operators | USDT and percent | Window is unavailable when the latest snapshot is stale, no historical snapshot exists inside the horizon tolerance window, or `portfolio_equity_usdt` is missing, invalid, or non-positive. Missing history never becomes zero, interpolated, backfilled, inferred from `portfolio`, or replaced with `open_value_usdt`. | realized PnL; price-only return; open value | Status: `implemented` in Django consumer; paired bot registry synchronization still needs verification when that repository is available. |
| `portfolio_status_equity_chart_7d_png` | Diagnostic | On-demand mobile chart of portfolio equity over the last seven days. | Render reliable 7-day canonical snapshot equity points as a PNG line chart | `bot.portfolio_snapshots.notes.portfolio_equity_usdt` only | Django `/portfolio_status` chart renderer | Telegram operators; reusable by future transports | PNG bytes | Unavailable when fewer than two usable 7-day canonical points exist or rendering/sending fails; generated images are not persisted. `open_value_usdt` is not a chart fallback. | historical change values | Transport-agnostic chart bytes; Telegram delivery is an adapter concern. |
