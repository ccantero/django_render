...
| `portfolio_status_unrealized_pnl_usdt` | KPI | Aggregate current unrealized PnL for material open lots, displayed as `Unrealized now` in Telegram. | Sum `current value - lot cost basis` | `position_lots` quantity/entry price plus `portfolio.current_price` | Django `/portfolio_status` | Telegram operators | USDT | Unavailable when any required current price or material lot entry price is missing/non-positive. | realized PnL; historical 24h equity change | Projection valuation, not audited realization. It is a current open-position snapshot, not an explanation of 24h equity movement. |
...
| `portfolio_status_realized_closed_trades_utc` | KPI | Current UTC calendar-day realized closed-trade PnL displayed as `Realized PnL` in Telegram. | Sum `lot_closures.realized_pnl` for linked operations whose `executed_at`, falling back to `created_at` only when execution time is null, is inside `[00:00 UTC, next 00:00 UTC)` | `trade_operations` UTC operation timestamp window plus linked `lot_closures.realized_pnl` | Django `/portfolio_status` | Telegram operators | USDT | Unavailable when realized PnL evidence cannot be read; renders `+0.00 USDT` when the read succeeds but no closed-trade PnL exists. | full realized PnL audit; Binance "Today's PNL"; historical 24h equity attribution | Uses the same current UTC calendar-day rule as Analytics and `/buy_status`, not a rolling previous-24h or snapshot-delta attribution. |
